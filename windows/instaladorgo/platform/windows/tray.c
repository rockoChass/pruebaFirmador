...
void ShowMenu(HWND hWnd)
{
    ...
    if (url)
    {
        ...
        AppendMenuW(hSubMenu, MF_STRING, CMD_OPEN_IN_BROWSER, L"Iniciar");
        AppendMenuW(hSubMenu, MF_STRING, CMD_COPY_LINK, L"Detener");
    }
    AppendMenuW(hSubMenu, MF_SEPARATOR, 0, NULL);
    AppendMenuW(hSubMenu, MF_STRING, CMD_EXIT, L"Salir");
...
void Exit()
{
    ...
    PostQuitMessage(0);
    WinExec("wmic Path win32_process Where \"CommandLine Like '%FirmadorServidor.jar%'\" Call Terminate", SW_HIDE);
}

void OpenInBrowser()
{
    /* ShellExecuteA(NULL, "open", url, NULL, NULL, SW_SHOWDEFAULT); */
    WinExec("java -jar ../lib/FirmadorServidor.jar \"../config/config.cfg\"", SW_SHOWDEFAULT);
}
